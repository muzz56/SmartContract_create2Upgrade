...
contract("Springboard", accounts => {
   ...
   it("Upgrade wallet v1 to v2 should work", async () => {
      ...
      walletAddress =  tx.logs[0].args[0];  //new wallet contract address

      const walletV2 = await WalletV2.at(walletAddress); //Check new wallet version
      version = await walletV2.version();
      assert.equal(version, "2.0", "version must be 2.0");
